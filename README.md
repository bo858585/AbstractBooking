...
***Техническое задание (детальное описание) и руководство по установке системы
выполнения заказов с фиксированной ценой. Если заказ другой разновидности
(например, почасовая оплата и т.д.), вводимая сумма - гарантия безопасной сделки,
остальные подробности указываются в сообщении заказа (отредактировано в
http://dillinger.io)***
...
исполняющегося. Для каждого заказа в таблице выводится его
...
создавшего заказ, в таблице есть кнопка "Удалить заказ", доступаная при статусах
заказа "Ожидает выполнения", “Завершен”.
...
(Надпись на кнопке - статус заказа):**
  ...
  - "Удалить заказ" - "Ожидает выполнения", “Завершен” (статус "Завершен"
  отображается только на странице своих заказов пользователя, на странице общих
  заказов заказ после завершения не отображается)
 ...
 **Схема отображения кнопок/действий в столбце таблицы:**
  ...
  - Строка таблицы в общем списке заказов не отображается, если статус заказа
  "completed".
